...
# how to use?
# crontab -e
# */10 * * * * curl -fsSL http://192.168.2.88:18080/lens/static/downloads/tingyuncheck/tingyun_check_sys.sh | sh >/dev/null 2>&1
########################################################################
...
# GET INIT
source ~/.bashrc && INIT_LIST=(mysql redis es kafka report frontend backend screen)   #配置服务字段列表 service list
...
frontend() {
	...
	[ -z "$frontend_pr" ] && check_alert frontend 0401001 || check_over frontend 0401001
}
redis() {
...
backend() {
	backend_pr=`ps ax | grep "DappName=dc-sys-backend" | egrep -v "grep"`
	[ -z "$backend_pr" ] && check_alert backend 0402001 || check_over backend 0402001
}
report() {
