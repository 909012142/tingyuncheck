...
# how to use?
# crontab -e
# */10 * * * * curl -fsSL http://192.168.2.88:18080/lens/static/downloads/tingyuncheck/tingyun_check_app.sh | sh >> /dev/null 2>&1
########################################################################
...
#0107001       app	traceindex	app产品webviewindex服务状态异常
#0108001       app	tracesearch	app产品webviewsearch服务状态异常
...
# GET INI
source ~/.bashrc && INIT_LIST=(mysql redis es  kafka report frontend wrap backend aggr alarm traceindex tracesearch nbfs active sensors screen)   #配置服务字段列表 app list active需要额外添加
...
frontend() {
	frontend_pr=`ps ax | grep java | grep frontend | egrep -v "grep"`
	[ -z "$frontend_pr" ] && check_alert frontend 0101001 || check_over frontend 0101001
}
redis() {
...
wrap() {
	wrap_pr_app=`ps ax | grep "DappName=dc-backend-app-wrapping"| egrep -v "grep"`
	[ -z "$wrap_pr_app" ] && check_alert wrap 0102001 || check_over wrap 0102001
}
backend() {
	backend_pr_app=`ps ax | grep "DappName=dc-backend-app" | grep -v "DappName=dc-backend-app-sensors" | grep -v "DappName=dc-backend-app-wrapping" | egrep -v "grep"`
	[ -z "$backend_pr_app" ] &&  check_alert backend 0103001 || check_over backend 0103001
}
aggr() {
	aggr_pr_hourly=`ps ax | grep "DappName=dc-aggr-app-hourly" | egrep -v "grep"`
	aggr_pr_daily=`ps ax | grep "DappName=dc-aggr-app-daily" | egrep -v "grep"`
	aggr_pr_baseline=`ps ax | grep "DappName=dc-backend-dynamic-baseline" | egrep -v "grep"`
	aggr_pr_devices=`ps ax | grep "DappName=dc-aggr-active-devices" | egrep -v "grep"`
	[ -z "$aggr_pr_hourly" ] || [ -z "$aggr_pr_daily" ] || [ -z "$aggr_pr_baseline" ] || [ -z $aggr_pr_devices ] && check_alert aggr 0104001 || check_over aggr 0104001
}
alarm() {
	alarm_pr=`ps ax | grep "DappName=dc-alarm" | egrep -v "grep"`
	[ -z "$alarm_pr" ] && check_alert alarm 0105001 || check_over alarm 0105001
}
traceindex() {
	webviewindex_pr=`ps ax | grep "DappName=app-webview-trace-index" | egrep -v "grep"`
	[ -z "$webviewindex_pr" ] && check_alert webviewindex 0107001 || check_over webviewindex 0107001
}
tracesearch() {
	webviewsearch_pr=`ps ax | grep "DappName=app-webview-trace-search" | egrep -v "grep"`
	[ -z "$webviewsearch_pr" ] && check_alert webviewsearch 0108001 || check_over webviewsearch 0108001
}
sensors() {
	sensors_pr=`ps ax | grep "DappName=dc-backend-app-sensors" | egrep -v "grep"`
	[ -z "$sensors_pr" ] && check_alert sensors 0106001 || check_over sensors 0106001
}
active() {
	active_pr=`ps ax | grep "DappName=dc-active-region-app" | egrep -v "grep"`
	[ -z "$active_pr" ] && check_alert active 0110001 || check_over active 0110001
}
report() {
...
nbfs() {
	nbfs_pr=`ps ax | grep java | grep nbfs | egrep -v "grep"`
	[ -z "$nbfs_pr" ] &&  check_alert nbfs 0109001 || check_over nbfs 0109001
}
screen() {
