...
# how to use?
# crontab -e
# */10 * * * * curl -fsSL http://192.168.2.88:18080/lens/static/downloads/tingyuncheck/tingyun_check_server.sh | sh >/dev/null 2>&1
########################################################################
...
# GET INIT
source ~/.bashrc && INIT_LIST=(frontend redis es kafka backend hotspot alarm report mysql screen)   #配置服务字段列表 service list
...
frontend() {
	...
	[ -z "$frontend_pr" ] && check_alert frontend 0201001 || check_over frontend 0201001
}
redis() {
...
backend() {
	backend_pr_server=`ps ax | grep "DappName=dc-backend-server" | egrep -v "grep"`
	backend_pr_hourly=`ps ax | grep "DappName=dc-aggr-server-hourly" | egrep -v "grep"`
	backend_pr_daily=`ps ax | grep "DappName=dc-aggr-server-daily" | egrep -v "grep"`
	backend_pr_baseline=`ps ax | grep "DappName=dc-backend-dynamic-baseline" | egrep -v "grep"`
	[ -z "$backend_pr_server" ] || [ -z "$backend_pr_hourly" ] || [ -z "$backend_pr_daily" ] || [ -z "$backend_pr_baseline" ] && check_alert backend 0202001 || check_over backend 0202001 
}
hotspot() {
	hotspot_pr=`ps ax | grep "DappName=dc-hotspot" | egrep -v "grep"`
	[ -z "$hotspot_pr" ] && check_alert hotspot 0204001 || check_over hotspot 0204001
}
alarm() {
	alarm_pr=`ps ax | grep "DappName=dc-alarm" | egrep -v "grep"`
	[ -z "$alarm_pr" ] && check_alert alarm 0203001 || check_over alarm 0203001
}
report() {
